...
Cylinder::Cylinder(float x, float y, float z, float radius, float half_height, const std::string &name, Material *mat) : GeometryObject(mat, name), _cx(x), _cy(y), _cz(z), _radius(radius), _half_height(half_height) { }
