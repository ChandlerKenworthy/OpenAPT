...
Box::Box(float x, float y, float z, float half_width_x, float half_width_y, float half_width_z, const std::string &name, Material *mat) : GeometryObject(mat, name), _cx(x), _cy(y), _cz(z), _hwx(half_width_x), _hwy(half_width_y), _hwz(half_width_z) { }
